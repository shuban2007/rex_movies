...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (pathname === '/api/search-multi') {
    ...
    try {
      ...
      res.statusCode = 500;
      res.end(JSON.stringify({ error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch search results.' } }));
      return;
    }
  }
  ...
  if (pathname.startsWith('/api/tv/') && !pathname.includes('/season/')) {
    ...
    if (!id || isNaN(Number(id))) {
      res.statusCode = 400;
      res.end(JSON.stringify({ error: { code: 'INVALID_ID', message: 'Valid TV ID required.' } }));
      return;
    }
    ...
    try {
      ...
      res.statusCode = 500;
      res.end(JSON.stringify({ error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch TV details.' } }));
      return;
    }
  }
  ...
  if (pathname.startsWith('/api/tv/') && pathname.includes('/season/')) {
    ...
    if (!id || isNaN(Number(id)) || !seasonNumber || isNaN(Number(seasonNumber))) {
      res.statusCode = 400;
      res.end(JSON.stringify({ error: { code: 'INVALID_ID', message: 'Valid TV ID and season number required.' } }));
      return;
    }
    ...
    try {
      ...
      res.statusCode = 500;
      res.end(JSON.stringify({ error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch season details.' } }));
      return;
    }
  }
